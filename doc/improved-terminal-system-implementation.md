...
- 共有リンク（QR）:
  ...
  - 起動時に `scripts/print-share-qr.sh` が共有URL+QRを自動出力
  - Tailscale前提で `TERMINAL_PUBLIC_BASE_URL` は MagicDNS を自動採用（未指定時）
...
| Phase 9 | PWA対応 | 4時間 | 58時間 |
| Phase 10 | UI洗練・デザイン調整 | 5時間 | 63時間 |
| Phase 11 | デプロイ（プロキシなし） | 4時間 | 67時間 |
| Phase 12 | 総合テスト・バグフィックス | 6時間 | 73時間 |
...
- [ ] ターミナル基本操作（入力・表示）
- [ ] 日本語IME入力
- [ ] 特殊キー操作（Ctrl, Alt, Esc等）
- [ ] tmux操作（ウィンドウ作成・切り替え・分割）
- [ ] セッション管理UI（ウィンドウ一覧・切り替え）
- [ ] PWAインストール
- [ ] オフライン時の適切なエラー表示
...
| 4週目 | Phase 9-12 | 73時間 | PWA + 総合テスト + リリース |
...
| PWA登録失敗 | 低 | 中 | next-pwa公式ガイド参照、実機テスト |

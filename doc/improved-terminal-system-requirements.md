...
- クリーンでモダンなダークモードデザイン
- モバイルファースト（タッチ操作優先）
- **コントロールパネル（ControlPanel）**: 特殊キーボタン（Esc, Tab, Enter, ^C, ^D, ^Z）、矢印キー（逆T字配置）、IME入力ボタン（常時表示）
- 全画面モード対応
- QRコード共有モーダル

### 5.5 tmux操作パネル（削除済み）

v3で一度復活したTmuxPanelは、運用を経てv4で再度削除された。

**削除理由**:
- tmuxはセッション維持（ブラウザを閉じてもシェルが生き続ける）のためだけに使用しており、ウィンドウ分割・切り替え等のtmux機能はユーザーが実際に使っていなかった
- tmuxはバックエンドで透過的に動作しており、ユーザーがtmuxを意識する必要がない
- UIをシンプルに保つため、ControlPanelのみの構成に統一

**現在の構成**: ControlPanel（Esc, Tab, Enter, ^C, ^D, ^Z, 矢印キー, IME）のみ
...
| 対象 | 理由 |
|------|------|
| **TmuxPanel（tmux操作パネル）** | tmuxはセッション維持のためだけに使用。ユーザーがtmux操作を直接行う必要がなく、UIのシンプル化のため削除 |
| **+ボタン（新規タブ）** | 同一tmuxセッションへの重複接続で、実用的な意味がなかったため削除 |

### v3 → v4 で変更したもの

| 対象 | v3 | v4 | 変更点 |
|------|-----|-----|--------|
| **UIパネル構成** | ControlPanel + TmuxPanel（排他表示） | ControlPanelのみ（常時表示） | tmuxは裏方で透過的に動作。パネル切り替えの複雑さを排除 |
| **tmuxの位置づけ** | ユーザーが操作するGUI対象 | バックエンドの裏方（ユーザーから不可視） | セッション維持機能のみ利用 |

...
### 1.2 成功基準
- [ ] 起動ごとにランダムなログイントークンを自動発行できる（固定も可能）
- [ ] スマホで手入力せずにログインできる（ワンタイム共有URL/QR）
- [ ] モダンで直感的なUI（Claude Code on the Web風）
- [ ] 日本語IME入力が快適に動作
- [ ] PWA対応でアプリライクに使用可能
- [ ] tmux操作が視覚的に分かりやすい
- [ ] セキュアな認証と通信
...
- **共有リンク（QR）**
  - ログイン済みユーザーが **ワンタイム（1回のみ）**の共有トークンを発行できる
...
### 7.1 技術選定
- [ ] フロントエンドフレームワーク: React vs Vue
- [ ] WebSocket vs SSE（どちらが適切か）
- [ ] SSH接続のプロキシ方法（直接 vs Go/Node.jsブリッジ）

### 7.2 機能仕様
- [x] トークンの更新方法（起動ごとランダム + ワンタイム共有リンクでカバー）
- [ ] 複数デバイスからの同時接続を許可するか
- [ ] セッション切断時の自動再接続機能の必要性
- [ ] ファイル転送機能の優先度

### 7.3 デザイン詳細
- [ ] 具体的な配色（Claude風 or 独自路線）
- [ ] アイコンセット（Font Awesome, Material Icons等）
- [ ] アニメーション効果の有無
